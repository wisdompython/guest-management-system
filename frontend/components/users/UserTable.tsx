...
export function UserTable({
  ...
  return (
    <div className="divide-y" style={{ borderColor: 'var(--line)' }}>
      {users.map((u) => (
        <UserRow key={u.id} u={u} meId={meId} deletingId={deletingId} onDelete={onDelete} onEditSave={onEditSave} />
      ))}
    </div>
  )
}

function UserRow({ u, meId, deletingId, onDelete, onEditSave }: {
  u: AuthUser
  meId: number | undefined
  deletingId: number | null
  onDelete: (user: AuthUser) => void
  ...
  const [form, setForm] = useState<EditForm>({ first_name: u.first_name, last_name: u.last_name, email: u.email, role: u.role, is_active: u.is_active, password: '' })
  ...
  function cancelEdit() { setExpanded(false); setFormError('') }
    ...
    <div>
      ...
      <div className="flex items-center gap-4 px-5 py-4 transition-colors hover:bg-[var(--bg)]">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <button onClick={() => expanded ? cancelEdit() : openEdit()}
            className="text-xs font-semibold transition hover:opacity-70"
            style={{ color: expanded ? 'var(--muted)' : 'var(--brand)' }}>

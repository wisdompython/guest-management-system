        ...
        {loading ? (
          ...
          <Link href={`/admin/events/${ev.id}`} key={ev.id} className="flex items-center justify-between px-5 py-3 transition hover:bg-[var(--chip)]"
            ...
            <span className="text-[13px] font-bold tabular-nums" style={{ color: 'var(--ink)' }}>
              {ev.rsvp_workflow_id ? ev.estimated_guest_count : ev.allow_plus_one ? ev.estimated_guest_count : ev.guest_count}
              {(ev.rsvp_workflow_id || ev.allow_plus_one) && <span className="ml-1 text-[10px] font-normal" style={{ color: 'var(--muted)' }}>est.</span>}
            </span>
          </Link>
        ))}
      </div>

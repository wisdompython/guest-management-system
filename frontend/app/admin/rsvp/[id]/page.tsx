...
import { api, RsvpRecipient, RsvpRecipientOrdering, RsvpRecipientSegment, RsvpResponseStatus, RsvpWorkflow } from '@/lib/api'
...
export default function RsvpWorkflowDetailPage() {
  ...
  const [recipientOrdering, setRecipientOrdering] = useState<RsvpRecipientOrdering>('name')
  ...
  }, [id, page, recipientOrdering, responseFilter, search, segmentFilter])
  ...
  async function retrySingle(kind: 'invitation' | 'pass', recipientId: number) {
    ...
    try {
      ...
      setNotice(`Retry queued with template “${result.template}”${result.template_changed ? ' (new template detected)' : ''}.`)
      await Promise.all([loadWorkflow(), loadRecipients()])
    } catch (err) {
  ...
  function exportFilteredRecipients() {
    const params = new URLSearchParams()
    if (responseFilter !== 'all') params.set('response_status', responseFilter)
    if (segmentFilter !== 'all') params.set('segment', segmentFilter)
    if (search.trim()) params.set('search', search.trim())
    window.location.href = `${BASE_URL}/rsvp/workflows/${id}/export/?${params}`
  }
  ...
  const hasActiveRecipientFilters = responseFilter !== 'all' || segmentFilter !== 'all' || !!search.trim()
  ...
  return (
    <div className="px-6 py-6 lg:px-8 lg:py-7">
      ...
      <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div><div className="flex flex-wrap items-center gap-2"><h1 className="text-xl font-bold" style={{ color: 'var(--ink)' }}>{workflow.event_name}</h1><span className="rounded-full px-2.5 py-1 text-[11px] font-semibold capitalize" style={{ background: workflow.status === 'active' ? 'var(--success-bg)' : 'var(--brand-soft)', color: workflow.status === 'active' ? 'var(--success)' : 'var(--brand)' }}>{workflow.status}</span></div><p className="mt-1 text-sm" style={{ color: 'var(--muted)' }}>{new Date(workflow.event_date).toLocaleString('en-GB', { dateStyle: 'long', timeStyle: 'short' })}{workflow.response_deadline ? ` · Replies close ${new Date(workflow.response_deadline).toLocaleDateString('en-GB')}` : ''}</p><p className="mt-1 text-[11px]" style={{ color: 'var(--muted)' }}>Invitation template: {workflow.invitation_template_name || 'Not configured'} · Pass template: {workflow.pass_template_name || 'Not configured'}</p></div>
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => { window.location.href = `${BASE_URL}/rsvp/workflows/${id}/export/?delivery_status=failed` }} className="rounded-lg px-4 py-2 text-xs font-semibold" style={{ border: '1px solid rgba(239,68,68,0.35)', color: 'var(--danger)' }}>Export failed</button>
          ...
          {workflow.status === 'active' && <><button disabled={!!working || stats.awaiting === 0} onClick={() => { if (confirm(`Remind guests whose RSVP invitation was delivered or read but who have not responded? Failed and unsent invitations will not be retried.`)) runAction('remind', async () => { const result = await api.remindAwaitingRsvpGuests(id); setNotice(`${result.queued} reminder${result.queued === 1 ? '' : 's'} queued`) }) }} className="rounded-lg px-4 py-2 text-xs font-semibold disabled:opacity-40" style={{ border: '1px solid var(--line)', color: 'var(--ink)' }}>Remind awaiting</button><button disabled={!!working || stats.delivery_failed === 0} onClick={() => { setSegmentFilter('delivery_failed'); setResponseFilter('all'); setPage(1); setNotice('Select failed invitations or passes below, then choose the matching retry action.'); }} className="rounded-lg px-4 py-2 text-xs font-semibold disabled:opacity-40" style={{ border: '1px solid rgba(239,68,68,0.35)', color: 'var(--danger)' }}>Retry failed sends ({stats.delivery_failed})</button><button disabled={!!working} onClick={() => runAction('pause', () => api.pauseRsvpWorkflow(id))} className="rounded-lg px-4 py-2 text-xs font-semibold" style={{ border: '1px solid var(--line)', color: 'var(--muted)' }}>Pause</button></>}
      ...
      <div className="mt-5 rounded-[12px] p-5" style={{ background: 'var(--panel)', border: '1px solid var(--line)' }}>
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-[11px]" style={{ color: 'var(--muted)' }}><span>● <b style={{ color: 'var(--success)' }}>Confirmed</b> {stats.confirmed}</span><span>● <b style={{ color: 'var(--danger)' }}>Declined</b> {stats.declined}</span><span>● Awaiting response {stats.awaiting}</span><span>● Failed delivery {stats.invitation_failed}</span><span>● Not delivered {stats.not_sent}</span></div>
      </div>

      <div className="mt-5 overflow-hidden rounded-[12px]" style={{ background: 'var(--panel)', border: '1px solid var(--line)' }}>
        <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between" style={{ borderBottom: '1px solid var(--line)' }}><div><h2 className="text-sm font-semibold">Recipients</h2><p className="mt-0.5 text-xs" style={{ color: 'var(--muted)' }}>{recipientCount} matching guests</p></div><div className="flex flex-wrap gap-2"><input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} placeholder="Search guest…" className="min-w-[170px] flex-1 rounded-lg px-3 py-2 text-xs focus:outline-none" style={{ background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }} /><select value={responseFilter} onChange={(e) => { setResponseFilter(e.target.value as 'all' | RsvpResponseStatus); setPage(1) }} className="rounded-lg px-3 py-2 text-xs focus:outline-none" style={{ background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}><option value="all">All responses</option><option value="awaiting">Awaiting</option><option value="confirmed">Confirmed</option><option value="declined">Declined</option></select><select value={segmentFilter} onChange={(e) => { setSegmentFilter(e.target.value as 'all' | RsvpRecipientSegment); setPage(1) }} className="rounded-lg px-3 py-2 text-xs focus:outline-none" style={{ background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}><option value="all">All delivery states</option>{(Object.keys(SEGMENT_LABEL) as RsvpRecipientSegment[]).map((segment) => <option key={segment} value={segment}>{SEGMENT_LABEL[segment]}</option>)}</select><select value={recipientOrdering} onChange={(e) => { setRecipientOrdering(e.target.value as RsvpRecipientOrdering); setPage(1) }} aria-label="Sort recipients" className="rounded-lg px-3 py-2 text-xs focus:outline-none" style={{ background: 'var(--bg)', border: '1px solid var(--line)', color: 'var(--ink)' }}><option value="name">Guest name</option><option value="-invitation_sent_at">Date sent: newest</option><option value="invitation_sent_at">Date sent: oldest</option><option value="-confirmed_at">Confirmed: newest</option><option value="confirmed_at">Confirmed: oldest</option><option value="-last_message_sent_at">Last message: newest</option><option value="last_message_sent_at">Last message: oldest</option></select><button disabled={!hasActiveRecipientFilters} onClick={exportFilteredRecipients} className="rounded-lg px-3 py-2 text-xs font-semibold disabled:opacity-40" style={{ border: '1px solid var(--line)', color: 'var(--brand)' }}>Export filtered ({recipientCount})</button></div></div>
...
function deliveryCell(status: string, error: string, autoRetries: number, templateName: string, attemptedAt: string | null) {
  const failed = status === 'failed'
  return <>
    <span className="text-xs capitalize" style={{ color: failed ? 'var(--danger)' : 'var(--muted)' }}>{status.replace('_', ' ')}</span>
    {status === 'queued' && attemptedAt && <p className="mt-0.5 text-[10px]" style={{ color: 'var(--muted)' }}>Queued {new Date(attemptedAt).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })} · awaiting worker</p>}
    ...
    {failed && templateName && <p className="mt-0.5 max-w-[240px] text-[10px]" style={{ color: 'var(--muted)' }}>Last template: {templateName}{attemptedAt ? ` · ${new Date(attemptedAt).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}` : ''}</p>}
  </>
}
...
function RecipientRow({ recipient, retry, working, selected, onToggle }: { recipient: RsvpRecipient; retry: (kind: 'invitation' | 'pass') => void; working: string; selected: boolean; onToggle: () => void }) {
  ...
  return <tr style={{ borderBottom: '1px solid var(--line)' }}><td className="w-10 px-4 py-3"><input type="checkbox" aria-label={`Select ${recipient.guest_name}`} disabled={!selectable} checked={selected} onChange={onToggle} /></td><td className="px-4 py-3"><p className="font-semibold">{recipient.guest_name}</p><p className="mt-0.5 text-[11px]" style={{ color: 'var(--muted)' }}>{recipient.ticket_type || 'Guest'}{recipient.table_number ? ` · Table ${recipient.table_number}` : ''}{recipient.is_plus_one ? ` · Plus one of ${recipient.primary_guest_name}` : recipient.plus_one_attending ? ' · Bringing a plus one' : ''}{recipient.aso_ebi_requested ? ` · Aso Ebi ${recipient.aso_ebi_quantity} yd` : ''}</p></td><td className="px-4 py-3"><span className="text-xs font-semibold" style={{ color: responseColor }}>{RESPONSE_LABEL[recipient.response_status]}</span></td><td className="px-4 py-3">{deliveryCell(recipient.invitation_status, invitationError, recipient.invitation_auto_retries, recipient.invitation_last_template_name, recipient.invitation_queued_at)}</td><td className="px-4 py-3">{deliveryCell(recipient.pass_status, passError, recipient.pass_auto_retries, recipient.pass_last_template_name, recipient.pass_queued_at)}</td><td className="px-4 py-3 text-xs" style={{ color: 'var(--muted)' }}>{recipient.responded_at ? new Date(recipient.responded_at).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' }) : '—'}</td><td className="px-4 py-3 text-right">{recipient.invitation_status === 'failed' && <button disabled={!!working} onClick={() => retry('invitation')} className="text-xs font-semibold disabled:opacity-40" style={{ color: 'var(--brand)' }}>{working === `invitation-${recipient.id}` ? 'Queuing…' : 'Retry invitation'}</button>}{recipient.pass_status === 'failed' && <button disabled={!!working} onClick={() => retry('pass')} className="text-xs font-semibold disabled:opacity-40" style={{ color: 'var(--brand)' }}>{working === `pass-${recipient.id}` ? 'Queuing…' : 'Retry pass'}</button>}</td></tr>
}

    ...
    <div className="px-6 py-6 lg:px-8 lg:py-7">
      ...
      <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {workflow.status === 'active' && <><button disabled={!!working || stats.awaiting === 0} onClick={() => { if (confirm(`Send or retry RSVP invitations for eligible awaiting guests? Delivered invitations respect the reminder cooldown, and failed sends respect their retry cooldown.`)) runAction('remind', async () => { const result = await api.remindAwaitingRsvpGuests(id); const parts = [`${result.queued} invitation${result.queued === 1 ? '' : 's'} queued`]; if (result.skipped_cooldown) parts.push(`${result.skipped_cooldown} failed send${result.skipped_cooldown === 1 ? '' : 's'} still in retry cooldown`); setNotice(parts.join(' · ')) }) }} className="rounded-lg px-4 py-2 text-xs font-semibold disabled:opacity-40" style={{ border: '1px solid var(--line)', color: 'var(--ink)' }}>Send / remind awaiting</button><button disabled={!!working} onClick={() => runAction('pause', () => api.pauseRsvpWorkflow(id))} className="rounded-lg px-4 py-2 text-xs font-semibold" style={{ border: '1px solid var(--line)', color: 'var(--muted)' }}>Pause</button></>}
